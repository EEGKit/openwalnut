...
#include "WMGlassBrain.xpm"

#include "WMGlassBrain.h"

// This line is needed by the module loader to actually find your module. Do not remove. Do NOT add a ";" here.
W_LOADABLE_MODULE( WMGlassBrain )

WMGlassBrain::WMGlassBrain():
    WModule()
{
}
...
boost::shared_ptr< WModule > WMGlassBrain::factory() const
{
    // See "src/modules/template/" for an extensively documented example.
    return boost::shared_ptr< WModule >( new WMGlassBrain() );
}

const char** WMGlassBrain::getXPMIcon() const
{
    return glassbrain_xpm;
}
const std::string WMGlassBrain::getName() const
{
    return "GlassBrain";
}

const std::string WMGlassBrain::getDescription() const
{
    return "Glass brain drawn around a visualization.";
}

void WMGlassBrain::connectors()
{
    // takes a scalar dataset (T1 recommended)
...

{
    ...
    m_alpha  = m_properties->addProperty( "Focus-Dependent Transparency", "The amount of transparency depending on depth.", 0.0 );
    m_alpha->setMin( 0.0 );
    m_alpha->setMax( 2.0 );

    m_viewAxial = m_properties->addProperty( "Inferior or Superior opaque", "Opaque context on top (+1), underneath (-1), or off (0).", -1 );
    ...
    m_viewSagittal = m_properties->addProperty( "Left or Right opaque", "Opaque context on the right (+1), on the left (-1), or off (0).", 1 );
    ...
    m_phong  = m_properties->addProperty( "Phong Shading", "If enabled, Phong shading gets applied on a per-pixel basis.", false );
...
void WMGlassBrain::requirements()
{
}

void WMGlassBrain::moduleMain()
{
    // the WGEShader can take the name of any glsl shader (without extension) in the shaders folder
    m_shader = osg::ref_ptr< WGEShader > ( new WGEShader( "WMGlassBrain", m_localPath ) );
    ...
    m_moduleState.add( WKernel::getRunningKernel()->getSelectionManager()->getPropAxialPos()->getUpdateCondition() );
    m_moduleState.add( WKernel::getRunningKernel()->getSelectionManager()->getPropCoronalPos()->getUpdateCondition() );
    m_moduleState.add( WKernel::getRunningKernel()->getSelectionManager()->getPropSagittalPos()->getUpdateCondition() );
    ...
    while( !m_shutdownFlag() )
    {
        ...
        if ( dataValid )
        {
            ...
            WGEColormapping::apply( cube, m_shader, 4 );
