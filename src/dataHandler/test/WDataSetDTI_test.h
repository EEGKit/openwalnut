...
public:
    ...
    void testInstanziation( void )
    {
        ...
        boost::shared_ptr< WValueSetBase > newValueSet( new WValueSet< float >( 1, 6,
            boost::shared_ptr< std::vector< float > >( new std::vector< float >( data ) ),
            W_DT_FLOAT ) );
    ...
    void testTensorAccess( void )
    {
        ...
        std::vector< float > data( &dataArray[0], &dataArray[0] + sizeof( dataArray ) / sizeof( float ) );
        boost::shared_ptr< WValueSetBase > newValueSet( new WValueSet< float >( 1, 6,
            boost::shared_ptr< std::vector< float > >( new std::vector< float >( data ) ), W_DT_FLOAT ) );
        boost::shared_ptr< WGrid > newGrid( new WGridRegular3D( 1, 1, 1, 1, 1, 1 ) );
