...
public:
    ...
    {
        int a[4] = { 0, -5, 1, 2 };
        boost::shared_ptr< std::vector< int8_t > > v = boost::shared_ptr< std::vector< int8_t > >(
    ...
    void testGetWValue( void )
    {
        ...
        for ( std::size_t idx = 0; idx < v->size()/dim; idx++ )
        {
          ...
          for ( std::size_t i = 0; i < dim; i++ ) currentWValue[ i ] = v->at( idx*dim + i );
    ...
    void testSubArrayInstantiation()
    {
        ...
        WValueSet< int8_t > set( 1, 2,
            boost::shared_ptr< std::vector< int8_t > >( new std::vector< int8_t >( v ) ),
            W_DT_INT8 );
